...
1. Python が `python/output/backtest/...` に成果物を出力する
2. backend が `ResultStore` を通じて成果物とメタデータを解決する
...
├── python/                 # 分析ロジック、CLI、設定、出力、requirements
├── scripts/                # 補助スクリプト
├── tests/                  # 共有 fixture など
...
成果物は主に `python/output/` 配下へ保存されます。
...
2. `ResultStore` に manifest キャッシュやバージョニングを追加する
