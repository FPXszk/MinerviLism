...
import { ExperimentListControls } from './ExperimentListControls'
import { useExperimentListFilters } from '../hooks/useExperimentListFilters'
...
export const ExperimentListTable: React.FC<ExperimentListTableProps> = ({
  ...
  React.useEffect(() => {
    if (strategyFilter === 'all' || strategyOptions.includes(strategyFilter)) {
      return
    }
...
const tableStyles = `
  ...
  .experiment-reset-button:disabled {
    cursor: not-allowed;
    opacity: 0.6;
  }
